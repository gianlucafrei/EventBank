...
class CardsLoadTest {
	...
	final int numberOfPayments = 1_000;
	...
	void loadTest() throws InterruptedException {
		...
		long sagasCount = paymentService.cardPaymentSagas.values().stream().count();
		long openSagasCount = Long.MAX_VALUE;
		long failedSagasCount = Long.MAX_VALUE;

		while (openSagasCount > 0){

			openSagasCount = paymentService.cardPaymentSagas.values().stream().filter(s -> s.isOpen()).count();
			failedSagasCount = paymentService.cardPaymentSagas.values().stream().filter(s -> s.isFailed()).count();
