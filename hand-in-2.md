...
While the coding part in this hand in was not as extensive as in the last one, it was nice to see how powerful the Kafka Streams are. Only little code is required to alter messages, create extensive queries and much more. Especially, grouping, aggregating, and windowing data to create tables that can be queried opens a lot of possibilities. Hence, it was nice experimenting with the Kafka Streams despite some setup troubles. Our main hurdles were the Serdes, we both had much difficulties with the custom serdes because of the CloudEvent message envelope and combining that with Avro. It would have probably made sense to get an example without the message specification working first and add it on top afterwards. 

Overall, the course technologies and concepts were very well-chosen and exciting to work with. Maybe, the lecture could have expanded on scalability a bit more with code examples. Nevertheless, it was a fun and interesting project and course to work at because there are so many trade-offs to consider and new frameworks to apply. 